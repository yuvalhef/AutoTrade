...
class TradingEnv(gym.Env):
    ...
    def __init__(self, initial_balance=10000, commission=0.0025, reward_func='sortino', **kwargs):
        ...
        self.data_files = [f for f in listdir('./data') if isfile(join('./data', f))]
        data = random.choice(self.data_files)
        ...
        self.window = 100  # k observations backwords
    ...
    def reset(self):
        ...
        benchmarks = self.kwargs.get('benchmarks', [])
        self.benchmarks = [
            {
                'label': 'Buy and HODL',
                'values': buy_and_hodl(self.df['Close'], self.initial_balance, self.commission)
            },
            {
                'label': 'RSI Divergence',
                'values': rsi_divergence(self.df['Close'], self.initial_balance, self.commission)
            },
            {
                'label': 'SMA Crossover',
                'values': sma_crossover(self.df['Close'], self.initial_balance, self.commission)
            },
            *benchmarks,
        ]
